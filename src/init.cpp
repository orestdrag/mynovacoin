...
{
    uiInterface.ThreadSafeMessageBox(str, _("BCCA"), CClientUIInterface::BTN_OK | CClientUIInterface::MODAL);
    return false;
}

bool static InitWarning(const std::string &str)
{
    uiInterface.ThreadSafeMessageBox(str, _("BCCA"), CClientUIInterface::BTN_OK | CClientUIInterface::ICON_WARNING | CClientUIInterface::MODAL);
    return true;
}
...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: bcca.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: bccad.pid)") + "\n";
...
/*     if (mapArgs.count("-connect") && mapMultiArgs["-connect"].size() > 0)
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. BCCA is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s.  BCCA is probably already running."), strDataDir.c_str()));
    
    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();

    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("BCCA version %s (%s)\n", FormatFullVersion().c_str(), CLIENT_DATE.c_str());
    ...
    if (fDaemon)
    {
        fprintf(stdout, "BCCA server starting\n");
        fflush(stdout);
    };
    ...
    ********************************************************* */
    ...
    if (fs::exists(GetDataDir() / strWalletFileName))
    {
        ...
        if (r == CDBEnv::RECOVER_OK)
        {
            ...
            uiInterface.ThreadSafeMessageBox(msg, _("BCCA"), CClientUIInterface::BTN_OK | CClientUIInterface::ICON_WARNING | CClientUIInterface::MODAL);
        };
...
/*     if (!fNoListen)
    ...
    if (nLoadWalletRet != DB_LOAD_OK)
    {
        ...
        {
            strErrors << _("Error loading wallet.dat: Wallet requires newer version of BCCA") << "\n";
        } else
        if (nLoadWalletRet == DB_NEED_REWRITE)
        {
            strErrors << _("Wallet needed to be rewritten: restart BCCA to complete") << "\n";
