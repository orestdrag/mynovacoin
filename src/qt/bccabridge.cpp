...
#include "bccabridge.moc"

BCCABridge::BCCABridge(BCCAGUI *window, QObject *parent) :
    QObject         (parent),
    window          (window),
...
void BCCABridge::setClientModel()
{
...
// This is just a hook, we won't really be setting the model...
void BCCABridge::setWalletModel()
{
...
// This is just a hook, we won't really be setting the model...
void BCCABridge::setMessageModel()
{
    populateMessageTable();
    connectSignals();
}

void BCCABridge::copy(QString text)
{
    QApplication::clipboard()->setText(text);
}

void BCCABridge::paste()
{
    emitPaste(QApplication::clipboard()->text());
}

// Options
void BCCABridge::populateOptions()
{
...
// Transactions
bool BCCABridge::addRecipient(QString address, QString label, QString narration, qint64 amount, int txnType, int nRingSize)
{
...
void BCCABridge::clearRecipients()
{
    recipients.clear();
}

bool BCCABridge::sendCoins(bool fUseCoinControl, QString sChangeAddr)
{
    int inputTypes = -1;
    int nAnonOutputs = 0;
    int ringSizes = -1;
...
void BCCABridge::openCoinControl()
{
...
void BCCABridge::updateCoinControlAmount(qint64 amount)
{
    CoinControlDialog::payAmounts.clear();
...
void BCCABridge::updateCoinControlLabels(unsigned int &quantity, int64_t &amount, int64_t &fee, int64_t &afterfee, unsigned int &bytes, QString &priority, QString low, int64_t &change)
{
    emitCoinControlUpdate(quantity, amount, fee, afterfee, bytes, priority, low, change);
}

QVariantMap BCCABridge::listAnonOutputs()
{
...
void BCCABridge::populateTransactionTable()
{
...
void BCCABridge::updateTransactions(QModelIndex topLeft, QModelIndex bottomRight)
{
...
void BCCABridge::insertTransactions(const QModelIndex & parent, int start, int end)
{
    // New Transactions...
    transactionModel->populateRows(start, end);
}

QString BCCABridge::transactionDetails(QString txid)
{
...
// Addresses
void BCCABridge::populateAddressTable()
{
...
void BCCABridge::updateAddresses(QModelIndex topLeft, QModelIndex bottomRight)
{
    addressModel->poplateRows(topLeft.row(), bottomRight.row());
}

void BCCABridge::insertAddresses(const QModelIndex & parent, int start, int end)
{
...
QString BCCABridge::newAddress(QString addressLabel, int addressType, QString address, bool send)
{
...
QString BCCABridge::lastAddressError()
{
    QString sError;
...
QString BCCABridge::getAddressLabel(QString address)
{
    return addressModel->atm->labelForAddress(address);
}

void BCCABridge::updateAddressLabel(QString address, QString label)
{
...
bool BCCABridge::validateAddress(QString address)
{
    return window->walletModel->validateAddress(address);
}

bool BCCABridge::deleteAddress(QString address)
{
    return addressModel->atm->removeRow(addressModel->atm->lookupAddress(address));
}

// Messages
void BCCABridge::appendMessages(QString messages, bool reset)
{
    emitMessages("[" + messages + "]", reset);
}

void BCCABridge::appendMessage(int row)
{
...
void BCCABridge::populateMessageTable()
{
...
void BCCABridge::insertMessages(const QModelIndex & parent, int start, int end)
{
    while(start <= end)
    {
        appendMessage(start++);
...
bool BCCABridge::deleteMessage(QString key)
{
    return window->messageModel->removeRow(thMessage->mtm->lookupMessage(key));
}

bool BCCABridge::markMessageAsRead(QString key)
{
    return window->messageModel->markMessageAsRead(key);
}

QString BCCABridge::getPubKey(QString address)
{
    return addressModel->atm->pubkeyForAddress(address);;
}

bool BCCABridge::setPubKey(QString address, QString pubkey)
{
...
bool BCCABridge::sendMessage(const QString &address, const QString &message, const QString &from)
{
    WalletModel::UnlockContext ctx(window->walletModel->requestUnlock());
...
QString BCCABridge::createGroupChat(QString label)
{
...
QString BCCABridge::joinGroupChat(QString privkey, QString label)
{
    /*
...
QVariantList BCCABridge::inviteGroupChat(QString qsaddress, QVariantList invites, QString from)
{
...
void BCCABridge::connectSignals()
{
...
QString BCCABridge::translateHtmlString(QString string)
{
    int i = 0;
...
QVariantMap BCCABridge::userAction(QVariantMap action)
{
...
// Blocks
QVariantMap BCCABridge::listLatestBlocks()
{
...
QVariantMap BCCABridge::findBlock(QString searchID)
{
...
QVariantMap BCCABridge::blockDetails(QString blkHash)
{
...
QVariantMap BCCABridge::listTransactionsForBlock(QString blkHash)
{
...
QVariantMap BCCABridge::txnDetails(QString blkHash, QString txnHash)
{
...
QVariantMap BCCABridge::signMessage(QString address, QString message)
{
    QVariantMap result;
...
QVariantMap BCCABridge::verifyMessage(QString address, QString message, QString signature)
{
    QVariantMap result;
...
QVariantMap BCCABridge::getNewMnemonic(QString password, QString language)
{
    QVariantMap result;
...
QVariantMap BCCABridge::importFromMnemonic(QString inMnemonic, QString inPassword, QString inLabel, bool fBip44, int64_t nCreateTime)
{
...
QVariantMap BCCABridge::extKeyAccList() {
    QVariantMap result;
...
QVariantMap BCCABridge::extKeyList() {
    QVariantMap result;
...
QVariantMap BCCABridge::extKeyImport(QString inKey, QString inLabel, bool fBip44, int64_t nCreateTime)
{
    QVariantMap result;
    ...
    if (eKey58.Set58(sInKey.c_str()) == 0)
    {
        ...
        {
            result.insert("error_msg", "Import failed - Key must begin with BCCA prefix.");
            return result;
        }
...
QVariantMap BCCABridge::extKeySetDefault(QString extKeyID)
{
    QVariantMap result;
...
QVariantMap BCCABridge::extKeySetMaster(QString extKeyID)
{
    QVariantMap result;
    std::string sInKey = extKeyID.toStdString();
    if (extKeyID.length() == 0)
...
QVariantMap BCCABridge::extKeySetActive(QString extKeyID, QString isActive)
{
    QVariantMap result;
    std::string sInKey = extKeyID.toStdString();
