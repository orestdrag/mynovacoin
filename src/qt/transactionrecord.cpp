...
QString TransactionRecord::getTypeLabel(const int &type)
{
    ...
    case RecvWithAddress:
        return BCCAGUI::tr("Received with");
    case RecvFromOther:
        return BCCAGUI::tr("Received from");
    case SendToAddress:
    case SendToOther:
        return BCCAGUI::tr("Sent to");
    case SendToSelf:
        return BCCAGUI::tr("Payment to yourself");
    case Generated:
        return BCCAGUI::tr("Mined");
    case RecvBCCA:
        return BCCAGUI::tr("Received bcca");
    case SendBCCA:
        return BCCAGUI::tr("Sent bcca");
    case Other:
        return BCCAGUI::tr("Other");
    default:
        return "";
    }
}
...
QString TransactionRecord::getTypeShort(const int &type)
{
    ...
    case TransactionRecord::RecvBCCA:
        return "input";
    case TransactionRecord::SendToAddress:
    case TransactionRecord::SendToOther:
    case TransactionRecord::SendBCCA:
        return "output";
    default:
        return "inout";
    }
...
QList<TransactionRecord> TransactionRecord::decomposeTransaction(const CWallet *wallet, const CWalletTx &wtx)
{
    ...
    int64_t nCredBCCACOIN, nCredBCCA;
    wtx.GetCredit(nCredBCCACOIN, nCredBCCA, true);
    int64_t nCredit = nCredBCCACOIN + nCredBCCA;
    ...
    {
        if (nNet > 0 && nCredBCCA > 0)
        {
            // -- credit
            TransactionRecord sub(hash, nTime, TransactionRecord::RecvBCCA, "", "", nNet, 0);
        ...
        {
            // -- debit
            TransactionRecord sub(hash, nTime, TransactionRecord::SendBCCA, "", "", nNet, 0);
        ...
        for (unsigned int nOut = 0; nOut < wtx.vout.size(); nOut++)
        {
                ...
                && txout.IsAnonOutput())
            {
                ...
                if (wallet->HaveKey(ckidD))
                {
                    ...
                    sub.type = TransactionRecord::RecvBCCA;
    ...
    } else
    {
        ...
        BOOST_FOREACH(const CTxIn& txin, wtx.vin)
        {
                ...
                && txin.IsAnonInput())
            {
                ...
                {
                    fAllFromMe = false;
                    break; // display as send/recv bcca
                };
                continue;
            };
        ...
        if (fAllFromMe)
        {
            ...
            for (unsigned int nOut = 0; nOut < wtx.vout.size(); nOut++)
            {
                    ...
                    && txout.IsAnonOutput())
                {
                    ...
                    sub.type = TransactionRecord::SendBCCA;
                    sub.address = CBitcoinAddress(ckidD).ToString();
