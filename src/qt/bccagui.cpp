...
BCCAGUI::BCCAGUI(QWidget *parent):
    QMainWindow(parent),
    bridge(new BCCABridge(this)),
    clientModel(0),
    walletModel(0),
    messageModel(0),
    ...
    nWeight(0)
{
    ...
    resize(1280, 720);
    setWindowTitle(tr("BCCA") + " - " + tr("Client"));
#ifndef Q_OS_MAC
    qApp->setWindowIcon(QIcon(":icons/bcca"));
    setWindowIcon(QIcon(":icons/bcca"));
#else
...
#ifdef Q_OS_WIN
    QFile html("C:/bcca/index.html");
#else
    QFile html("/opt/bcca/index.html");
#endif
...
void BCCAGUI::pageLoaded(bool ok)
{
...
void BCCAGUI::addJavascriptObjects()
{
    documentFrame->addToJavaScriptWindowObject("bridge", bridge);

}

void BCCAGUI::urlClicked(const QUrl & link)
{
...
void BCCAGUI::createActions()
{
    ...
    aboutAction = new QAction(QIcon(":/icons/bcca"), tr("&About BCCA"), this);
    aboutAction->setToolTip(tr("Show information about BCCA"));
    aboutAction->setMenuRole(QAction::AboutRole);
    aboutQtAction = new QAction(QIcon(":/trolltech/qmessagebox/images/qtlogo-64.png"), tr("About &Qt"), this);
    aboutQtAction->setToolTip(tr("Show information about Qt"));
    aboutQtAction->setMenuRole(QAction::AboutQtRole);
    optionsAction = new QAction(QIcon(":/icons/options"), tr("&Options..."), this);
    optionsAction->setToolTip(tr("Modify configuration options for BCCA"));
    optionsAction->setMenuRole(QAction::PreferencesRole);
    toggleHideAction = new QAction(QIcon(":/icons/bcca"), tr("&Show / Hide"), this);
...
void BCCAGUI::setClientModel(ClientModel *clientModel)
{
    ...
    if (clientModel)
    {
        ...
        if (mode != NT_FULL)
        {
            ...
            setWindowTitle(tr("BCCA") + " - " + tr("Wallet") + ", " + sMode);
        };
...
#ifndef Q_OS_MAC
            qApp->setWindowIcon(QIcon(":icons/bcca_testnet"));
            setWindowIcon(QIcon(":icons/bcca_testnet"));
#else
            MacDockIconHandler::instance()->setIcon(QIcon(":icons/bcca_testnet"));
#endif
            if(trayIcon)
            {
                trayIcon->setToolTip(tr("BCCA client") + QString(" ") + tr("[testnet]"));
                trayIcon->setIcon(QIcon(":/icons/bcca_testnet"));
...
void BCCAGUI::setWalletModel(WalletModel *walletModel)
{
...
void BCCAGUI::setMessageModel(MessageModel *messageModel)
{
...
void BCCAGUI::createTrayIcon()
{
    ...
    trayIcon->setToolTip(tr("BCCA client"));
    trayIcon->setIcon(QIcon(":/icons/bcca"));
...
#ifndef Q_OS_MAC
void BCCAGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void BCCAGUI::setNumConnections(int count)
{
    ...
    connectionsIcon.setAttribute("data-title", tr("%n active connection(s) to BCCA network", "", count));
}

void BCCAGUI::setNumBlocks(int count, int nTotalBlocks)
{
...
void BCCAGUI::error(const QString &title, const QString &message, bool modal)
{
    // Report errors from network/worker thread
    if(modal)
    {
...
void BCCAGUI::changeEvent(QEvent *e)
{
...
void BCCAGUI::closeEvent(QCloseEvent *event)
{
    if(clientModel)
    {
#ifndef Q_OS_MAC // Ignored on Mac
...
void BCCAGUI::askFee(qint64 nFeeRequired, bool *payFee)
{
    QString strMessage =
        tr("This transaction is over the size limit.  You can still send it for a fee of %1, "
          "which goes to the nodes that process your transaction and helps to support the network.  "
          "Do you want to pay the fee?").arg(
                BitcoinUnits::formatWithUnit(BitcoinUnits::BCCACOIN, nFeeRequired));
...
void BCCAGUI::incomingTransaction(const QModelIndex & parent, int start, int end)
{
...
void BCCAGUI::incomingMessage(const QModelIndex & parent, int start, int end)
{
    if(!messageModel)
        return;
...
void BCCAGUI::optionsClicked()
{
    bridge->triggerElement("#navitems a[href=#options]", "click");
    showNormalIfMinimized();
}

void BCCAGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void BCCAGUI::dragMoveEvent(QDragMoveEvent *event)
{
    event->accept();
}

void BCCAGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
        ...
        if (nValidUrisFound)
            ...
            notificator->notify(Notificator::Warning, tr("URI handling"), tr("URI can not be parsed! This can be caused by an invalid BCCA address or malformed URI parameters."));
    }

    event->acceptProposedAction();
}

void BCCAGUI::handleURI(QString strURI)
{
...
void BCCAGUI::setEncryptionStatus(int status)
{
...
void BCCAGUI::encryptWallet(bool status)
{
    if(!walletModel)
        return;
...
void BCCAGUI::showNormalIfMinimized(bool fToggleHidden)
{
...
void BCCAGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void BCCAGUI::updateWeight()
{
    if (!pwalletMain)
        return;
...
void BCCAGUI::updateStakingIcon()
{
