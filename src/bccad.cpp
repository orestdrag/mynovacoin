...
bool AppInit(int argc, char* argv[])
{
    ...
    try
    {
        ...
        if (mapArgs.count("-?") || mapArgs.count("--help"))
        {
            ...
            std::string strUsage = _("BCCA version") + " " + FormatFullVersion() + "\n\n" +
                _("Usage:") + "\n" +
                  "  bccad [options]                     " + "\n" +
                  "  bccad [options] <command> [params]  " + _("Send command to -server or bccad") + "\n" +
                  "  bccad [options] help                " + _("List commands") + "\n" +
                  "  bccad [options] help <command>      " + _("Get help for a command") + "\n";
        ...
        for (int i = 1; i < argc; i++)
            if (!IsSwitchChar(argv[i][0]) && !boost::algorithm::istarts_with(argv[i], "bcca:"))
                fCommandLine = true;
