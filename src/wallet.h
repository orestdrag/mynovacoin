...
public:
    ...
    int64_t GetBCCADebit(const CTxIn& txin) const;
    int64_t GetBCCACredit(const CTxOut& txout) const;
    ...
    bool GetCredit(const CTransaction& tx, int64_t& nBCCACOIN, int64_t& nBCCA) const
    {
        nBCCACOIN = 0;
        nBCCA = 0;
        BOOST_FOREACH(const CTxOut& txout, tx.vout)
        {
            if (tx.nVersion == ANON_TXN_VERSION
                && txout.IsAnonOutput())
            {
                nBCCA += GetBCCACredit(txout);
            } else
                nBCCACOIN += GetCredit(txout);
            if (!MoneyRange(nBCCACOIN)
                || !MoneyRange(nBCCA))
                throw std::runtime_error("CWallet::GetCredit() : value out of range");
        }
        return true;
...
public:
    ...
    mutable bool fAvailableBCCACreditCached;
    mutable bool fCreditSplitCached;
    mutable int64_t nDebitCached;
    mutable int64_t nCreditCached;
    mutable int64_t nAvailableCreditCached;
    mutable int64_t nChangeCached;
    mutable int64_t nAvailableBCCACreditCached;
    
    mutable int64_t nCredBCCACOINCached;
    mutable int64_t nCredBCCACached;
    ...
    void Init(const CWallet* pwalletIn)
    {
        ...
        fAvailableBCCACreditCached = false;
        
        nCredBCCACOINCached = 0;
        nCredBCCACached = 0;
    ...
    bool GetCredit(int64_t& nCredBCCACOIN, int64_t& nCredBCCA, bool fUseCache=true) const
    {
        // Must wait until coinbase is safely deep enough in the chain before valuing it
        if ((IsCoinBase() || IsCoinStake()) && GetBlocksToMaturity() > 0)
        {
            nCredBCCACOIN = nCredBCCA = 0;
            return true;
        }
        ...
        if (!fUseCache || !fCreditSplitCached)
        {
            nCredBCCACOINCached = 0;
            nCredBCCACached = 0;
            pwallet->GetCredit(*this, nCredBCCACOINCached, nCredBCCACached);
            fCreditSplitCached = true;
        };
        
        nCredBCCACOIN = nCredBCCACOINCached;
        nCredBCCA = nCredBCCACached;
        return true;
    }
    ...
    int64_t GetAvailableBCCACredit(bool fUseCache=true) const
    {
        // Must wait until coinbase is safely deep enough in the chain before valuing it
        if ((IsCoinBase() || IsCoinStake()) && GetBlocksToMaturity() > 0)
            return 0;
        
        if (fUseCache && fAvailableBCCACreditCached)
            return nAvailableBCCACreditCached;
        ...
        for (unsigned int i = 0; i < vout.size(); i++)
        {
            if (!IsSpent(i))
            {
                ...
                if (!MoneyRange(nCredit))
                    throw std::runtime_error("CWalletTx::GetAvailableBCCACredit() : value out of range");
            };
        };
        
        nAvailableBCCACreditCached = nCredit;
        fAvailableBCCACreditCached = true;
        return nCredit;
    };
