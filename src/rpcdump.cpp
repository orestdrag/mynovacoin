...
Value importprivkey(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 4)
        throw runtime_error(
            "importprivkey <bccaprivkey> [label] [rescan=true] [timestamp=1]\n"
...
Value dumpprivkey(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "dumpprivkey <bccaaddress/public_key_hex>\n"
            "Reveals the private key corresponding to <bccaaddress>.");
...
Value dumpwallet(const Array& params, bool fHelp)
{
    ...
    // produce output
    file << strprintf("# Wallet dump created by BCCA %s (%s)\n", CLIENT_BUILD, CLIENT_DATE);
